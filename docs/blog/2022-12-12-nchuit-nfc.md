...
PN532.js 這個程式是由均民所開發的開源專案，可以在 Google Chrome 上面透過藍牙 BLE 來跟讀卡機進行連線。
...
然後在讀卡機上放一張卡片，用 Google Chrome 開啟以下的網頁，然後請選擇「透過藍牙 BLE 連線」的方式，然後按下讀取卡片：

* <https://taichunmin.idv.tw/pn532.js/m1-uid4b-writer.html>

![](https://i.imgur.com/9AMAAVy.png)

如果你是在手機上第一次使用 BLE 功能，你可能會需要開啟「藍牙」、「定位」、「鄰近位置」等權限。接下來 Chrome 會要你選擇要連線的裝置，選擇裝置以後，按下配對按鈕：
...
PN532.js 這個程式除了支援 BLE 連線之外，還支援透過 USB Serial 進行連線。

#### 準備讀卡機及驅動程式

讀卡機可以使用外面賣的現成讀卡機，或是使用 [PN532 開發板](https://www.taiwansensor.com.tw/product/pn532-nfc-rfid-v3-%E8%BF%91%E5%A0%B4%E9%80%9A%E4%BF%A1%E6%A8%A1%E7%B5%84/)以及一條 USB 轉 TTL 的轉接線 (最常見的型號是 `CH340`)。
...
讀取到卡號以後，當然就是要把讀取到的卡號寫到專門用來拷貝用的特殊卡片，這個均民會在接下來的內容中詳細介紹。
...
在中國魔術卡問世以後，讀卡機廠商也會嘗試去偵測卡片是不是中國魔術卡，於是網路上就出現了各種不同的中國魔術卡來嘗試繞過讀卡機的偵測機制，以下會說明幾種市面上常見的中國魔術卡。
