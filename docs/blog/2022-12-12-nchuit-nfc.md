...
### [RFID Tools](https://play.google.com/store/apps/details?id=com.rfidresearchgroup.rfidtools)

支援以 USB OTG 或藍芽的方式控制支援的 NFC 讀卡機。

### [Credit Card Reader NFC (EMV)](https://play.google.com/store/apps/details?id=com.github.devnied.emvnfccard)
...
因為卡號可以隨意讀取，為了破解這類系統，市面上也因此出現了可以被修改卡號的特殊卡，以下是一些常見的種類：

* 低頻卡：T5577
* 高頻卡：中國魔術卡 (可拷貝 Mifare)

要修改這類的特殊卡，你可能需要特殊的程式及讀卡機。
...
### 使用 PN532.js 透過 BLE 來讀取卡號

PN532.js 這個程式是由均民所開發的開源專案，可以在 Google Chrome 上面透過 Web Serial API 以及 Web Bluetooth API 來跟讀卡機進行連線。

#### 準備一個支援 BLE 的 PN532 讀卡機

以下是均民在淘寶上買到的現成 BLE 讀卡機：
...
如果是淘寶買的現成讀卡機，就先把電源開關打開，如果是自行製作的讀卡機，就需要接上 `VCC` 以及 `GND`。

#### 開啟 PN532.js 的範例程式

請把讀卡機接到電腦上，然後在讀卡機上放一張卡片，用 Google Chrome 開啟以下的網頁，然後請選擇「透過藍牙 BLE 連線」的方式，然後按下讀取卡片：

* <https://taichunmin.idv.tw/pn532.js/m1-uid4b-writer.html>

![](https://i.imgur.com/9AMAAVy.png)

如果你是在手機上第一次使用 BLE 功能，你可能會需要開啟「藍牙」、「定位」、「鄰近位置」等權限。接下來 Chrome 會要你選擇要連線的裝置，選擇裝置以後，按下連線按鈕：

![](https://i.imgur.com/Ruzlny0.png)

如果在網頁上成功顯示十六進位的 UID，就代表成功讀取到卡號囉！

### 使用 PN532.js 透過 USB 來讀取卡號

PN532.js 這個程式是由均民所開發的開源專案，可以在 Google Chrome 上面透過 Web Serial API 以及 Web Bluetooth API 來跟讀卡機進行連線。

#### 準備讀卡機及驅動程式

讀卡機可以使用 [PN532 開發板](https://www.taiwansensor.com.tw/product/pn532-nfc-rfid-v3-%E8%BF%91%E5%A0%B4%E9%80%9A%E4%BF%A1%E6%A8%A1%E7%B5%84/)以及一條 USB 轉 TTL 的轉接線 (最常見的型號是 `CH340`)。

![](https://i.imgur.com/E6UANgh.jpg)

如果你的 `CH340` 是接腳的版本，你可以用四條杜邦線進行連結：

![](https://i.imgur.com/UKbUMcd.jpg)

如果你的 Windows 沒有辦法辨識 `CH340`，就可能需要[安裝 `CH340` 的驅動程式](https://www.wch.cn/downloads/CH341SER_EXE.html)：
...
請把讀卡機接到電腦上，然後在讀卡機上放一張卡片，用 Google Chrome 開啟以下的網頁，然後請選擇「透過 USB Serial 連線」的方式，然後按下讀取卡片：
...
目前 Proxmark3 有兩大主流的開源韌體，均民比較推薦使用 `Iceman/RRG` 版本的韌體，所以通常拿到 Proxmark3 第一步就是先把韌體刷成這個版本，如果你是用 Windows 的話，推薦可以在以下的頁面下載熱心網友包好的執行環境，均民在寫這篇文章時的最新版本是 `v4.15864`：
...
這類使用卡號的系統其實非常不安全，因為任何讀卡機都能讀到卡號，也有很多系統會把卡號當作公開資料對待，例如：去 7-11 儲值悠遊卡，收據上會直接把卡號印出來。
...
Mifare Classic 所使用的 [Crypto 1 加密技術](https://en.wikipedia.org/wiki/Crypto-1) 是在 1994 年被發明的，但大約在 2008 年時就有針對這個加密技術的攻擊方式被公開。雖然 NXP 有釋出新的卡片 (MIFARE Classic EV1) 來應對，但大約在 2015 年時又被發現新的攻擊方式。
...
成功逆推出卡片的金鑰後，我們就一樣使用 `hf mf autopwn --1k -s 2 -b -k a9ac67832330` 指令破解卡片。

## 拷貝卡片

現在我們成功把卡片金鑰破解出來後，接下來我們就可以開始複製卡片。通常這類可以修改卡號的特殊硬體都需要特別購買，以下會介紹幾種常見的類型。

### 中國魔術卡 (Chinese Magic Card)

為了確保卡號的唯一性及安全性，一張正常的 Mifare Classic 卡片在出廠時，卡號是無法進行修改的，但是網路上有人製作了可以修改製造商區塊的卡片，這類卡片通稱為中國魔術卡，因為卡號儲存於製造商區塊，只要修改這類卡片的製造商區塊，就能夠做到完整複製 Mifare Classic 的卡片。

在中國魔術卡問世以後，就會有讀卡機廠商嘗試去偵測卡片是不是中國魔術卡，於是網路上就出現了各種不同的中國魔術卡來嘗試繞過讀卡機的偵測機制，以下會說明幾種市面上常見的中國魔術卡。

> 詳細的中國魔術卡介紹可以看[網友整理的文件](https://github.com/RfidResearchGroup/proxmark3/blob/master/doc/magic_cards_notes.md)。

#### 中國魔術卡 Gen1a (UID)

這類卡片有提供後門指令，只要使用後門指令後，就允許讀寫任何資料。由於這種卡片問世的時間最早，所以很多讀卡機都知道如何偵測這類卡片。目前市面上有很多這類卡片的變種，有的變種也會偷工減料，例如不支援 `increment/decrement/restore/transfer` 指令。

#### 中國魔術卡 Gen2 (CUID)

這類卡片的製造商區塊會受到存取權限的影響。所以只要有該區塊的存取權限以及金鑰就可以修改。由於這種卡片問世的時間比 Gen1a 晚，相對之下會偵測這類卡片的讀卡機比較少。

#### 中國魔術卡 Gen2 (FUID, UFUID)

這類卡片的特性是，在鎖定前製造商區塊會受到存取權限的影響，在鎖定後製造商區塊就無法被修改，`FUID` 卡是製造商區塊在寫入一次之後自動鎖定，`UFUID` 卡則是提供了一個不可逆的鎖定指令。這種卡片因為鎖定後就跟正常卡片一樣，所以理論上在鎖定後讀卡機是無法偵測的。

#### 中國魔術卡 Gen4 (GTU)

這類卡片提供很多後門指令，允許模擬成多種不同的 Mifare 卡片，後門指令需要密碼驗證，可以修改後門指令的密碼，這類卡片目前在市面上的售價是最貴的，但因為還不夠普及，所以在本文中也不會深入研究。
